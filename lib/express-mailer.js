...
exports.extend = function (app, options) {
  ...
  createRender = function (render) {
    return function (sendOptions, locals, callback) {
      if (!stubTransport) {
        stubTransport = nodemailer.createTransport({jsonTransport: true});
      };
      sendMail(sendOptions, locals, stubTransport, render, function(err, info) {
        var renderedResult;
        try {
          renderedResult = JSON.parse(info).html;
        } catch (e) {
          renderedResult = info;
        }
        callback(err, renderedResult);
      });
    };
  };
